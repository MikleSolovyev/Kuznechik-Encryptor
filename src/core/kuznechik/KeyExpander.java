...
public class KeyExpander {
    ...
    private static final byte[][] iterationConstants = {
            {
    ...
    public static void expandKey(byte[][] roundKeys, byte[][] keyIn) {
        roundKeys[0] = Arrays.copyOf(keyIn[0], keyIn[0].length);
        roundKeys[1] = Arrays.copyOf(keyIn[1], keyIn[1].length);

        byte[][] tempKey = new byte[][]{roundKeys[0], roundKeys[1]};

        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 8; j++) {
                tempKey = F.direct(tempKey, iterationConstants[i * 8 + j]);
            }

            roundKeys[2 * i + 2] = Arrays.copyOf(tempKey[0], tempKey[0].length);
            roundKeys[2 * i + 3] = Arrays.copyOf(tempKey[1], tempKey[1].length);
        }
    }

    //Test key expand (ГОСТ Р 34.12─2015 Приложение А.1.4)
    public static void testSelf() {
        byte[][] roundKeys = new byte[10][16];
        ...
        KeyExpander.expandKey(roundKeys, key);

        System.out.println("Test key expand (ГОСТ Р 34.12─2015 Приложение А.1.4)");
        System.out.println("\tInput key:\n\t" + Arrays.toString(key[0]) + " " + Arrays.toString(key[1]));

        for (int i = 0; i < roundKeys.length; i++) {
            System.out.println("\n\tK" + (i + 1) + ":\n\t" + Arrays.toString(roundKeys[i]));
        }
    ...
    private static class F {
        ...
        private static byte[][] direct(byte[][] keyIn, byte[] iterationConstant) {
            byte[] leftKey = X.direct(L.direct(S.direct(X.direct(keyIn[0], iterationConstant))), keyIn[1]);
            byte[] rightKey = Arrays.copyOf(keyIn[0], keyIn[0].length);

            return new byte[][]{leftKey, rightKey};
        }
    }
}
